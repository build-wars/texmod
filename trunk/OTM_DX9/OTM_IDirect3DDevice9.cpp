...
{
  if (--OTM_Reference==0) //if our counter drops to zero, the real device will be deleted, so we clean up before
  {
...
HRESULT OTM_IDirect3DDevice9::EndScene(void)
{
  if ( /*OTM_Client!=NULL &&*/ OTM_Client->BoolSaveSingleTexture && SingleTexture!=NULL && SingleTexture->CrossRef_D3Dtex!=NULL)
  {
    ...
    OSD_Font->DrawTextA(NULL, buffer, -1, &rct, DT_NOCLIP, OTM_Client->FontColour);
  }
  return(m_pIDirect3DDevice9->EndScene());
}
